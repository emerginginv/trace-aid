...
export const UsersManagementTab = ({
  ...
  const [inviteRole, setInviteRole] = useState<
    "admin" | "manager" | "investigator" | "vendor" | "owner"
  >("investigator");
  ...
  const [selectedUserForColor, setSelectedUserForColor] = useState<User | null>(
    null,
  );
    ...
    try {
      if (organization && inviteRole === "admin") {
        ...
        if (
          ...
          currentAdminUsers >= planLimits.max_admin_users
        ) {
          toast.error(
            `You've reached the maximum of ${planLimits.max_admin_users} admin users for your ${planLimits.name}. Please upgrade to add more admin users.`,
          );
          return;
        }
      }
      ...
      if (
        ...
        !isTrialActive(subscriptionStatus.trial_end) &&
        subscriptionStatus.status !== "active"
      ) {
        toast.error(
          "Your trial has expired. Please add a payment method to continue adding users.",
        );
        return;
      }
      ...
      if (inviteRole === "admin") {
        ...
        const currentAdminCount = users.filter((u) =>
          u.roles.includes("admin"),
        ).length;

        if (currentAdminCount >= planLimits.max_admin_users) {
          toast.error(
            `Maximum admin limit reached for ${planLimits.name} plan. Please upgrade your plan to add more admins.`,
          );
          setInviting(false);
          return;
        }
      ...
      // Send the invitation email as a separate call (same pattern as signup emails).
      ...
      const inviteLink = `${loginUrl.replace(
        "/login",
        "/accept-invite",
      )}?token=${inviteToken}`;
      ...
      const emailBody = `
        ...
        <ul style="list-style-type: none; padding-left: 0;">
          ...
          <li><strong>Temporary Password:</strong> ${invitePassword}</li>
        </ul>
  ...
  const handleRoleChange = async (
    userId: string,
    newRole: "admin" | "manager" | "investigator" | "vendor" | "owner",
  ) => {
    try {
                ...
                <DialogContent>
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      >
                        <SelectTrigger id="inviteRole" className="text-left py-2">
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                          ...
                          <SelectItem value="manager">
                            <div className="flex flex-col">
                              <span>Manager</span>
                              <span className="text-xs text-muted-foreground">
                                Case oversight, no user management
                              </span>
                            </div>
                          </SelectItem>
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">
                {users.filter((u) => u.roles.includes("admin")).length} admin
              ...
              {editingUser && (
                <>
                  ...
                  <Button
                    ...
                    size="sm"
                    onClick={() =>
                      handleViewAsUser(editingUser.id, editingUser.email)
                    }
                    className="flex-1"
                  >
                    View as User
      ...
      <Dialog open={colorDialogOpen} onOpenChange={setColorDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Select a color for{" "}
              {selectedUserForColor?.full_name || selectedUserForColor?.email}{" "}
          ...
          <div className="grid grid-cols-5 gap-3 py-4">
            {colorPalette.map((color) => (
              <button
                key={color}
                onClick={() =>
                  selectedUserForColor &&
                  handleColorChange(selectedUserForColor.id, color)
                }
                className="w-12 h-12 rounded-lg border-2 hover:scale-110 transition-transform"
                style={{
                  backgroundColor: color,
                  borderColor:
                    selectedUserForColor?.color === color
                      ? "#000"
                      : "transparent",
                }}
                title={color}
