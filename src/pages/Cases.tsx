...
const Cases = () => {
  ...
  const {
    ...
    sortDirection,
    handleSort
  } = useSortPreference("cases", "case_number", "desc");
  ...
  const {
    ...
    toggleColumn,
    resetToDefaults
  } = useColumnVisibility("cases-columns", COLUMNS);
  ...
  const fetchCases = async () => {
    ...
    try {
      ...
      const {
        ...
        `).eq("organization_id", organization.id).order("created_at", {
        ascending: false
      });
  ...
  const fetchFinancialData = async () => {
    ...
    try {
      ...
      const budgetPromises = cases.filter(c => c.budget_dollars && c.budget_dollars > 0).map(async c => {
        const {
          data,
          error
      ...
      const [{
        ...
        data: expenseData,
        error: expenseError
      }] = await Promise.all([supabase.from("time_entries").select("case_id, hours, total").in("case_id", caseIds), supabase.from("expense_entries").select("case_id, total").in("case_id", caseIds)]);
      if (timeError) {
  ...
  const handleDeleteConfirm = async () => {
    ...
    const timeoutId = setTimeout(async () => {
      const {
        error
      } = await supabase.from("cases").delete().eq("id", deletedId);
      if (error) {
        const pending = pendingDeletions.current.get(deletedId);
        if (pending) {
          setCases(prev => [...prev, pending.item]);
  ...
  const handleBulkStatusChange = useCallback(async (newStatusKey: string) => {
    ...
    const oldStatuses = new Map(selectedCases.map(c => [c.id, {
      status: c.status,
    ...
    // Optimistic update
    setCases(prev => prev.map(c => selectedIds.has(c.id) ? {
      ...c,
      status: displayName,
      status_key: newStatusKey
    } : c));
  ...
  // Bulk delete handler
  const pendingBulkDeletions = useRef<Map<string, {
    items: Case[];
    timeoutId: NodeJS.Timeout;
  }>>(new Map());
  ...
  const filteredCases = cases.filter(caseItem => {
    ...
    const matchesSearch = searchQuery === '' || caseItem.title.toLowerCase().includes(query) || caseItem.case_number.toLowerCase().includes(query) || caseItem.description?.toLowerCase().includes(query) || caseItem.reference_number?.toLowerCase().includes(query) || caseItem.reference_number_2?.toLowerCase().includes(query) || caseItem.reference_number_3?.toLowerCase().includes(query);
  ...
  const EXPORT_COLUMNS: ExportColumn[] = [{
    ...
    key: "created_at",
    label: "Created",
    format: v => v ? format(new Date(v), "MMM d, yyyy") : "-"
  }, {
    key: "due_date",
    label: "Due Date",
    format: v => v ? format(new Date(v), "MMM d, yyyy") : "-"
  ...
  return <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className={cn("cursor-pointer hover:shadow-md transition-shadow", statusTypeFilter === 'open' && "ring-2 ring-primary")} onClick={() => setStatusTypeFilter(statusTypeFilter === 'open' ? 'all' : 'open')}>
        ...
        <Card className={cn("cursor-pointer hover:shadow-md transition-shadow", statusFilter === 'active' && "ring-2 ring-primary")} onClick={() => setStatusFilter(statusFilter === 'active' ? 'all' : 'active')}>
          <CardContent className="p-4 flex items-center gap-4">
        ...
        <Card className={cn("cursor-pointer hover:shadow-md transition-shadow", statusFilter === 'on_hold' && "ring-2 ring-primary")} onClick={() => setStatusFilter(statusFilter === 'on_hold' ? 'all' : 'on_hold')}>
          <CardContent className="p-4 flex items-center gap-4">
        ...
        <Card className={cn("cursor-pointer hover:shadow-md transition-shadow", statusTypeFilter === 'closed' && "ring-2 ring-primary")} onClick={() => setStatusTypeFilter(statusTypeFilter === 'closed' ? 'all' : 'closed')}>
          <CardContent className="p-4 flex items-center gap-4">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <Select value={statusFilter} onValueChange={setStatusFilter}>
          ...
          <SelectContent>
            ...
            {executionStatuses.map(status => <SelectItem key={status.id} value={status.status_key}>{status.display_name}</SelectItem>)}
          </SelectContent>
        </Select>
        <ColumnVisibility columns={COLUMNS} visibility={visibility} onToggle={toggleColumn} onReset={resetToDefaults} />
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <ResponsiveButton icon={<Download className="h-4 w-4" />} label="Export" variant="outline" size="sm" className="h-10" />
          </DropdownMenuTrigger>
      ...
      {/* Bulk Actions Bar */}
      {selectedIds.size > 0 && <div className="flex flex-wrap items-center gap-3 px-4 py-3 bg-muted rounded-lg">
          <span className="text-sm font-medium">
            {selectedIds.size} case{selectedIds.size !== 1 ? 's' : ''} selected
          </span>
          <div className="flex flex-wrap gap-2">
            {hasPermission('edit_cases') && <DropdownMenu>
                <DropdownMenuTrigger asChild>
                ...
                <DropdownMenuContent>
                  {executionStatuses.map(status => <DropdownMenuItem key={status.id} onClick={() => handleBulkStatusChange(status.status_key)}>
            ...
            {hasPermission('delete_cases') && <Button variant="destructive" size="sm" onClick={() => setBulkDeleteDialogOpen(true)}>
      ...
      {cases.length === 0 ? <Card>
          ...
          <div className="overflow-x-auto">
            <Table>
              <TableHeader>
                <TableRow>
                  {canBulkAction && <TableHead className="w-12">
                      <Checkbox checked={selectedIds.size === sortedCases.length && sortedCases.length > 0} onCheckedChange={toggleSelectAll} aria-label="Select all" />
                    </TableHead>}
                  {isVisible("case_number") && <SortableTableHead column="case_number" label="Case Number" sortColumn={sortColumn} sortDirection={sortDirection} onSort={handleSort} />}
                  {isVisible("title") && <SortableTableHead column="title" label="Title" sortColumn={sortColumn} sortDirection={sortDirection} onSort={handleSort} />}
                  {isVisible("status") && <SortableTableHead column="status" label="Status" sortColumn={sortColumn} sortDirection={sortDirection} onSort={handleSort} />}
                  {isVisible("created_at") && <SortableTableHead column="created_at" label="Created" sortColumn={sortColumn} sortDirection={sortDirection} onSort={handleSort} />}
                  {isVisible("due_date") && <SortableTableHead column="due_date" label="Due Date" sortColumn={sortColumn} sortDirection={sortDirection} onSort={handleSort} />}
              ...
              return <TableRow key={caseItem.id} className={`cursor-pointer hover:bg-muted/50 ${isClosed ? 'opacity-60' : ''} ${selectedIds.has(caseItem.id) ? 'bg-muted/30' : ''}`} onClick={() => navigate(`/cases/${caseItem.id}`)} role="button" tabIndex={0} onKeyDown={e => {
                if (e.key === 'Enter' || e.key === ' ') {
                  e.preventDefault();
                  navigate(`/cases/${caseItem.id}`);
                }
              }}>
                    {canBulkAction && <TableCell onClick={e => e.stopPropagation()}>
                        <Checkbox checked={selectedIds.has(caseItem.id)} onCheckedChange={() => toggleSelect(caseItem.id)} aria-label={`Select case ${caseItem.case_number}`} />
                      </TableCell>}
                    {isVisible("case_number") && <TableCell className={`font-medium ${isClosed ? 'text-muted-foreground' : ''}`}>
                        {caseItem.case_number}
                      </TableCell>}
                    {isVisible("title") && <TableCell className={isClosed ? 'text-muted-foreground' : ''}>
                    ...
                    {isVisible("status") && <TableCell onClick={e => e.stopPropagation()}>
                        {hasPermission('edit_cases') && canViewExactStatus ? <InlineEditCell value={caseItem.status_key || caseItem.status || ''} options={statusOptions} type="select" onSave={newValue => handleInlineStatusChange(caseItem.id, newValue)} displayAs="badge" badgeStyle={getStatusStyle(caseItem.status_key || caseItem.status)} /> : <Badge className="border capitalize" style={getStatusStyle(caseItem.status_key || caseItem.status)}>
                            {canViewExactStatus ? getDisplayName(caseItem.status_key || '') || caseItem.status || 'Unknown' : getStatusByKey(caseItem.status_key || '')?.phase || caseItem.status || 'Unknown'}
                          </Badge>}
                      </TableCell>}
                    {isVisible("created_at") && <TableCell>{new Date(caseItem.created_at).toLocaleDateString()}</TableCell>}
                    ...
                    {isVisible("actions") && <TableCell onClick={e => e.stopPropagation()}>
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                            <Button variant="ghost" size="icon" className="h-8 w-8">
                              <MoreVertical className="h-4 w-4 align-right" />
                            </Button>
                          </DropdownMenuTrigger>
                          <DropdownMenuContent align="end">
                            ...
                            {hasPermission('edit_cases') && <DropdownMenuItem onClick={() => {
                        ...
                        setFormOpen(true);
                            ...
                            {hasPermission('delete_cases') && <>
                                ...
                                <DropdownMenuItem onClick={() => handleDeleteClick(caseItem.id)} className="text-destructive focus:text-destructive">
                                  <Trash2 className="h-4 w-4 mr-2" />
                                  Delete Case
                                </DropdownMenuItem>
                              </>}
      ...
      <CaseForm open={formOpen} onOpenChange={open => {
      setFormOpen(open);
      ...
      <ConfirmationDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen} onConfirm={handleDeleteConfirm} title="Delete Case" description="Are you sure you want to delete this case? This action cannot be undone." />

      <ConfirmationDialog open={bulkDeleteDialogOpen} onOpenChange={setBulkDeleteDialogOpen} onConfirm={handleBulkDeleteConfirm} title="Delete Selected Cases" description={`Are you sure you want to delete ${selectedIds.size} case${selectedIds.size !== 1 ? 's' : ''}? This action cannot be undone.`} />
    </div>;
};
