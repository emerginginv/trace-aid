...
DROP POLICY IF EXISTS "Users can insert picklists" ON picklists;

CREATE POLICY "Users can insert picklists"
ON picklists
FOR INSERT
WITH CHECK (
  -- Allow if user is in the organization OR organization_id is NULL (for auto-creation)
  is_org_member(auth.uid(), organization_id) OR organization_id IS NULL
);

-- Create trigger function to auto-create organization if needed
...
-- Add trigger to picklists to ensure organization is set
