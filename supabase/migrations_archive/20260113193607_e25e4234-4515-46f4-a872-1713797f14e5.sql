-- Function to calculate budget forecast including pending billing items
...
RETURNS TABLE (
  ...
  -- Hard cap flag - SYSTEM PROMPT 9: Hard caps may block approval later
  hard_cap BOOLEAN,
...
BEGIN
  ...
  -- Exclude pending billing items from actual consumption
  SELECT 
  ...
  WHERE case_id = p_case_id
    ...
    AND (status IS NULL OR status NOT IN ('pending', 'cancelled', 'voided'));
  
  -- Calculate pending billing items (SYSTEM PROMPT 9: NOT definitively consumed)
  SELECT 
  ...
  RETURN QUERY SELECT
    ...
    -- Forecast status flags (SYSTEM PROMPT 9: Pending may trigger warnings)
