-- Fix get_organization_sla_summary RPC function with correct column names
...
BEGIN
  ...
  IF NOT v_is_authorized THEN
    RAISE EXCEPTION 'Not authorized to view SLA summary';
  END IF;
  ...
  SELECT jsonb_build_object(
    'slas', (
      SELECT COALESCE(jsonb_agg(
        jsonb_build_object(
          ...
          'measurement_period', s.measurement_window,
          'is_active', s.enabled,
          'latest_measurement', (
            SELECT jsonb_build_object(
              'measured_value', sm.measured_value,
              'is_met', (sm.status = 'met'),
              'measured_at', sm.calculated_at
            )
    ...
    'recent_breaches', (
      ...
      FROM (
        SELECT jsonb_build_object(
          ...
          'metric', s.metric,
          'target_value', s.target_value,
          'actual_value', sb.measured_value,
          'breach_start', sb.period_start,
          'breach_end', sb.period_end,
    ...
    'account_health', (
      SELECT jsonb_build_object(
        'risk_level', COALESCE(chs.risk_level, 'healthy'),
